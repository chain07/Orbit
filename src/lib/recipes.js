...
export const Recipes = {
  ...
  // ----------------------
  insightTemplates: [
    // --- CATEGORY: NEXT BEST MOVE (Actionable - Tactical) ---
    {
      id: 'finish_line',
      type: 'recommendation',
      context: 'tactical',
      ...
      message: (metric, { value }) => pick([
        `🏁 So close! You only need ${(metric.goal - value).toFixed(0)} more ${metric.unit || 'units'} to hit your ${metric.label} goal.`,
        `🤏 Almost there. Just ${(metric.goal - value).toFixed(0)} ${metric.unit || 'units'} left for ${metric.label}.`,
        `🎯 Finish strong! Complete the last ${(metric.goal - value).toFixed(0)} ${metric.unit || 'units'} for ${metric.label}.`
      ]),
    },
    {
      id: 'easy_win',
      ...
      message: (metric) => pick([
        `⚡️ Quick win: Check off "${metric.label}" to build momentum right now.`,
        `✅ Easy victory: Mark "${metric.label}" as done.`,
        `🚀 Start your roll. Complete "${metric.label}" now.`
      ]),
    },
    {
      id: 'protect_streak',
      ...
      message: (metric, { streak }) => pick([
        `🛡️ Protect the chain! Complete ${metric.label} today to make it ${streak + 1} days in a row.`,
        `🔗 Don't break the chain. Log ${metric.label} to keep the ${streak}-day streak alive.`,
        `🔥 Keep the fire burning. Extend your ${metric.label} streak to ${streak + 1} days.`
      ]),
    },
    {
      id: 'weekend_warrior',
    ...
    // --- CATEGORY: STREAKS & MOMENTUM (Strategic) ---
    {
      id: 'streak_hat_trick',
      type: 'celebration',
      context: 'strategic',
    ...
    },
    {
      id: 'comeback_kid',
      type: 'motivation',
      context: 'tactical', // Tactical because it encourages resumption TODAY
    ...
    // --- CATEGORY: PERFORMANCE & TRENDS (Strategic) ---
    {
      id: 'goal_crushed',
      type: 'celebration',
      context: 'strategic',
    ...
    // --- CATEGORY: SMART ADJUSTMENTS (System Maintenance - Strategic) ---
    {
      id: 'raise_the_bar',
      type: 'adjustment',
      context: 'strategic',
    ...
    // --- CATEGORY: CORRELATIONS (Advanced - Strategic) ---
    {
      id: 'positive_synergy',
      type: 'insight',
      context: 'strategic',
